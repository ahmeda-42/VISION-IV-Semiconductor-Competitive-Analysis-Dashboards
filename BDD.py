# Vision IV BDD Semiconductor Performance Explorer
# VERSION 4.0
import dash
...
materials_data = pd.DataFrame({
    # Material
    'Material': ['Boron-Doped Diamond (BDD)', 'Si', 'SiC', 'GaN/Si', 'GaN/SiC', 'GaAs'],
    
    # General Operating Metrics 
    'Max Temperature': [950, 150, 600, 300, 400, 250],  # °C
    'Breakdown Voltage': [1950, 600, 1200, 1200, 1500, 400],  # V
    'Max Frequency': [1200, 20, 100, 200, 300, 250],  # GHz
    
    # Radiation Hardness Metrics 
    "TID Tolerance": [1e7, 1e5, 1e6, 5e5, 1e6, 1e5],  # rad(Si)
    "DDD Tolerance": [1e16, 1e12, 1e14, 5e13, 1e14, 1e12],  # n/cm²
    "LET Threshold": [95, 10, 20, 30, 35, 8],  # MeV·cm²/mg
    "Displacement Threshold Energy": [42, 13, 20, 19, 20, 10],  # eV
    
    # Other Metrics
    'Bandgap': [5.0, 1.12, 3.26, 3.4, 3.4, 1.42],  # eV
    'Breakdown Field': [975, 30, 350, 330, 330, 40],  # MV/m
    'Electron Mobility': [0.25, 0.14, 0.09, 0.1, 0.1, 0.85],  # m²/V·s
    'Base Power Density': [4900, 100, 1000, 1300, 1500, 200],  # W/cm²
    'Thermal Conductivity': [2000, 150, 490, 130, 250, 55],  # W/m·K
})
...
app.layout = html.Div([
    html.H1("Vision IV BDD Semiconductor Performance Explorer", style={'textAlign': 'center'}),
    ...
    html.Div([
        ...
        dcc.Slider(id='frequency-slider', min=0, max=1200, step=25, value=600,
                   tooltip={"placement": "bottom", "always_visible": True})
    ], style={'padding': 20}),
